...
"welcome","well","we'll","went","were","we're","weren't","we've","what","whatever","what's","when","whence","whenever","when's","where","whereafter","whereas","whereby","wherein","where's","whereupon","wherever","whether","which","while","whither","who","whoever","whole","whom","who's","whose","why","why's","will","willing","wish","with","within","without","wonder","won't","would","wouldn't","yes","yet","you","you'd","you'll","your","you're","yours","yourself","yourselves","you've","zero" };
        ...
        public Dictionary<string, int> WordsCount = new Dictionary<string, int>();

        public string[] TotalWords;
        public void CalculateOccurencesOfWords()
        {
            try
            {
                if (string.IsNullOrEmpty(Text)) throw new ArgumentNullException("Text");

                TotalWords = (TotalWords == null || TotalWords.Count() == 0)
                    ? Text.ToWordsArray() : TotalWords;


                int _count = 0;

                // to do: replace this code with linq group by. so no need any loop here
                foreach (var item in TotalWords)
                {

                    _count = TotalWords.Where(m => m.ToLower().Equals(item.ToLower())).Count();
                    if (!WordsCount.ContainsKey(item.ToLower()))
                    {
                        WordsCount.Add(item.ToLower(), _count);
                    }
        ...
        public string RemoveStopWords()
        {
            try
            {
                if (string.IsNullOrEmpty(this.Text)) throw new ArgumentNullException("text");

                TotalWords = (TotalWords == null || TotalWords.Count() == 0)
                   ? Text.ToWordsArray() : TotalWords;

               
                TotalWords = TotalWords.Where(m => !StopWords.Any(f => f == m.ToLower())).ToArray();
